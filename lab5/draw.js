        ...
        for (let j = 0; j < dirMatrix.length; j++) {
            if (dirMatrix[i][j]) {
                if (i === j) {
                    ...
                    drawLine(vertexes[i].x-10, vertexes[i].y+5, vertexes[j].x, vertexes[j].y, 1);
                    drawLine(vertexes[j].x+10, vertexes[j].y-5, vertexes[i].x, vertexes[i].y, 1); 
                }
...
export {drawDirGraph, drawVertex, drawLine};